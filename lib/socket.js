...
class SecureSocket {
  ...
  connect(token) {
    try {
      ...
      if (!backendUrl) {
        console.error('Socket: Backend URL not configured');
        return false;
      }
      ...
      const secureOptions = isRailway ? {
        secure: true,
        rejectUnauthorized: false, // Railway SSL workaround
        agent: false
      } : {
        secure: true,
      ...
      this.socket = io(backendUrl, {
        ...
        timeout: 30000,
        autoConnect: true,
        forceNew: false, // Important for Railway
        multiplex: true,
        ...
        // Performance
        perMessageDeflate: false, // Disable for stability
